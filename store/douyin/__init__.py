...
async def update_douyin_aweme(aweme_item: Dict):
    ...
    utils.logger.info(
        f"[store.douyin.update_douyin_aweme] douyin aweme id:{aweme_id}, title:{save_content_item.get('title')}, liked_count:{save_content_item.get('liked_count')}, collected_count:{save_content_item.get('collected_count')}, comment_count:{save_content_item.get('comment_count')}, share_count:{save_content_item.get('share_count')}"
    )
